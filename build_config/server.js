...
command
    ...
    .option('-P, --path [value]', 'Add path')
    .parse(process.argv);

// 创建服务
var httpServer = http.createServer(processRequest);

var port = command.port || 1080;
var publicPath = command.path || 'dist';

console.log('listen on port:' + port, 'with path:' + publicPath);
...
function processRequest(request, response) {
    ...
    // static文件夹的绝对路径
    var staticPath = path.resolve(ROOT_PATH, publicPath);
    // 获取资源文件的绝对路径
